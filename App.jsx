...
import {
  ...
  Lock, Loader2, Zap, RefreshCw, Building2, ShoppingBag, ExternalLink,
  Mail, Phone, CircleAlert
} from "lucide-react";
...
function AuthenticatedApp({ profile }) {
  ...
  const allPages = {
    ...
    clients:    <Clients    isAdmin={isAdmin} />,
...
function Sidebar({ page, setPage, isAdmin, profile }) {
  const allNav = [
    ...
    { id: "clients",    label: "Clients",     icon: Building2,       admin: true  },
...
// ═══════════════════════════════════════════════════════════════════
// CLIENTS  (admin) — every brand signed up via the portal: tenant +
// linked profiles + Shopify status + order/product counts. Click a
// row to drill into orders / products / team for that brand.
// ═══════════════════════════════════════════════════════════════════
function Clients({ isAdmin }) {
  const [rows, setRows]     = useState(null);   // null = loading
  const [err, setErr]       = useState(null);
  const [q, setQ]           = useState("");
  const [filter, setFilter] = useState("all");  // all | connected | not_connected
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const empty = { data: [] };
      const [pRes, oRes, cpRes] = ids.length === 0
        ? [empty, empty, empty]
        : await Promise.all([
            ...
            supabase.from("shopify_orders")
              .select("tenant_id, pod_status, total_price, shopify_created_at, synced_at")
              .in("tenant_id", ids),
      ...
      const products = cpRes.data || [];

      const enriched = (tenants || []).map(t => {
        const ts = profiles.filter(p => p.tenant_id === t.id);
        const os = orders.filter(o => o.tenant_id === t.id);
        const ps = products.filter(p => p.tenant_id === t.id);
        const owner = ts.find(p => p.role === "client") || ts[0] || null;
        const revenue = os.reduce((s, o) => s + (Number(o.total_price) || 0), 0);
        const lastSync = os.length
          ? os.map(o => o.synced_at).filter(Boolean).sort().slice(-1)[0] || null
          : null;
        const lastOrderAt = os.length
          ? os.map(o => o.shopify_created_at).filter(Boolean).sort().slice(-1)[0] || null
          : null;
        return {
          tenant: t,
          owner,
          ...
          draftCount: ps.filter(p => p.status === "draft").length,
          liveCount:  ps.filter(p => p.status !== "draft").length,
          revenue,
          lastSync,
          lastOrderAt,
        };
  ...
  // Top-line KPIs
  const kpi = useMemo(() => {
    if (!rows) return { total: 0, connected: 0, orders: 0, signupsWeek: 0 };
    const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    return {
      total:     rows.length,
      connected: rows.filter(r => r.tenant.shopify_domain).length,
      orders:    rows.reduce((s, r) => s + r.orderCount, 0),
      signupsWeek: rows.filter(r => new Date(r.tenant.created_at).getTime() > weekAgo).length,
    };
  }, [rows]);
  ...
  const visible = useMemo(() => {
    if (!rows) return [];
    let xs = rows;
    if (filter === "connected")     xs = xs.filter(r => r.tenant.shopify_domain);
    if (filter === "not_connected") xs = xs.filter(r => !r.tenant.shopify_domain);
    if (q.trim()) {
      const needle = q.trim().toLowerCase();
      xs = xs.filter(r =>
        (r.tenant.name || "").toLowerCase().includes(needle) ||
        (r.tenant.slug || "").toLowerCase().includes(needle) ||
        (r.owner?.email || "").toLowerCase().includes(needle) ||
        (r.owner?.name  || "").toLowerCase().includes(needle) ||
        (r.tenant.shopify_domain || "").toLowerCase().includes(needle)
      );
    }
    return xs;
  }, [rows, q, filter]);

  const openRow = openId ? rows?.find(r => r.tenant.id === openId) : null;

  return (
    <div>
      <PageHeader
        title="Clients"
        sub="Every brand signed up on the portal — Shopify status, orders, products."
        action={
          <button className="btn-ghost" onClick={load} disabled={rows === null}>
      ...
      <div className="disp-summary">
        <div className="ds-card">
          <div className="ds-label">BRANDS</div>
          <div className="ds-val">{kpi.total}<span>total</span></div>
          <div className="ds-sub">all tenants</div>
        </div>
        <div className="ds-card">
          <div className="ds-label">SHOPIFY CONNECTED</div>
          <div className="ds-val">{kpi.connected}<span>of {kpi.total}</span></div>
          <div className="ds-sub">have a synced store</div>
        </div>
        <div className="ds-card">
          <div className="ds-label">TOTAL ORDERS</div>
          <div className="ds-val">{kpi.orders.toLocaleString("en-IN")}<span>synced</span></div>
          <div className="ds-sub">across all brands</div>
        </div>
        <div className="ds-card">
          <div className="ds-label">NEW THIS WEEK</div>
          <div className="ds-val">{kpi.signupsWeek}<span>signups</span></div>
          <div className="ds-sub">last 7 days</div>
        </div>
      </div>
      ...
      <div className="cl-toolbar">
        <div className="cl-search">
          <Search size={13} />
          <input
            placeholder="Search by brand, owner, email, Shopify domain…"
            value={q}
        ...
        <div className="cl-filters">
          {[
            ...
            <button key={f.id}
              className={`cl-fchip ${filter === f.id ? "on" : ""}`}
      ...
      <section className="panel cl-panel">
        {rows === null ? (
          <div className="empty"><Loader2 className="spin" size={14}/> Loading clients…</div>
        ) : visible.length === 0 ? (
          <div className="empty">No brands match.</div>
        ) : (
          <table className="cl-table">
            ...
            <tbody>
              {visible.map(r => {
                const t = r.tenant;
                const connected = !!t.shopify_domain;
                return (
                  <tr key={t.id} className="cl-row" onClick={() => setOpenId(t.id)}>
                    <td>
                      <div className="cl-brand">
                        <span className="cl-brand-mark">{(t.name || "?").slice(0, 1).toUpperCase()}</span>
                        <div>
                          <div className="cl-brand-name">{t.name}</div>
                          <div className="cl-brand-slug">{t.slug}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                      {r.owner ? (
                        <div className="cl-owner">
                          <div className="cl-owner-name">{r.owner.name || "—"}</div>
                          <div className="cl-owner-email">{r.owner.email}</div>
                          {r.team.length > 1 && (
                            <div className="cl-owner-more">+{r.team.length - 1} more</div>
                          )}
                        </div>
                      ) : <span className="cl-muted">No team yet</span>}
                    </td>
                    <td className="cl-date">
                      {t.created_at ? new Date(t.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "2-digit" }) : "—"}
                    </td>
                    <td>
                      {connected ? (
                        <div className="cl-shop">
                          <span className="cl-dot cl-dot-ok"/> {t.shopify_domain}
                        </div>
                      ) : (
                        <span className="cl-shop-none"><CircleAlert size={11}/> Not connected</span>
                      )}
                    </td>
                    <td className="num mono">{r.orderCount.toLocaleString("en-IN")}</td>
                    <td className="num mono">
                      {r.productCount}
                      {r.draftCount > 0 && <span className="cl-draft-tag"> · {r.draftCount} draft</span>}
                    </td>
                    <td className="num mono">{r.revenue > 0 ? `₹${r.revenue.toLocaleString("en-IN", { maximumFractionDigits: 0 })}` : "—"}</td>
                    <td className="cl-row-chev"><ChevronRight size={14}/></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {openRow && (
        <ClientDetailModal row={openRow} onClose={() => setOpenId(null)} />
      )}
    </div>
  );
}

function ClientDetailModal({ row, onClose }) {
  const [tab, setTab] = useState("overview");   // overview | orders | products | team
  const t = row.tenant;
  const connected = !!t.shopify_domain;
  ...
  const recentOrders = useMemo(
    () => [...row.orders]
      .sort((a, b) => (b.shopify_created_at || "").localeCompare(a.shopify_created_at || ""))
      .slice(0, 25),
    [row.orders]
  );

  return (
    <Modal wide title={t.name} onClose={onClose}>
      <div className="cd-head">
        <div className="cd-head-l">
          <div className="cl-brand-mark cl-brand-mark-lg">{(t.name || "?").slice(0, 1).toUpperCase()}</div>
          <div>
            <div className="cd-name">{t.name}</div>
            <div className="cd-meta">
              <span className="mono">{t.id}</span>
              <span className="cd-sep">·</span>
              <span>Joined {t.created_at ? new Date(t.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : "—"}</span>
              {connected && (<>
                <span className="cd-sep">·</span>
                <span><span className="cl-dot cl-dot-ok"/> Shopify connected</span>
              </>)}
            </div>
          </div>
        </div>
      ...
      <div className="cd-tabs">
        {[
          { id: "overview", label: "Overview" },
          { id: "orders",   label: `Orders (${row.orderCount})` },
          { id: "products", label: `Products (${row.productCount})` },
          { id: "team",     label: `Team (${row.team.length})` },
        ].map(x => (
          <button key={x.id}
            className={`cd-tab ${tab === x.id ? "on" : ""}`}
            onClick={() => setTab(x.id)}>
            {x.label}
          </button>
        ))}
      ...
      {tab === "overview" && (
        <div className="cd-grid">
          <div className="cd-card">
            <div className="cd-card-l">CONTACT</div>
            {row.owner ? (
              <>
                <div className="cd-card-v">{row.owner.name || "—"}</div>
                <div className="cd-card-sub"><Mail size={11}/> {row.owner.email}</div>
              </>
            ) : <div className="cd-card-sub">No owner profile linked.</div>}
          </div>
          <div className="cd-card">
            <div className="cd-card-l">SHOPIFY</div>
            {connected ? (
              <>
                <div className="cd-card-v mono">{t.shopify_domain}</div>
                <div className="cd-card-sub">
                  Last sync: {row.lastSync ? new Date(row.lastSync).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}
                </div>
                <a className="cd-card-link" href={`https://${t.shopify_domain}/admin`} target="_blank" rel="noopener noreferrer">
                  Open in Shopify <ExternalLink size={11}/>
                </a>
              </>
            ) : <div className="cd-card-sub">Not connected. Client hasn't pasted a token yet.</div>}
          </div>
          <div className="cd-card">
            <div className="cd-card-l">ORDERS</div>
            <div className="cd-card-v">{row.orderCount.toLocaleString("en-IN")}</div>
            <div className="cd-card-sub">
              Revenue: ₹{row.revenue.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
              {row.lastOrderAt && <> · Last: {new Date(row.lastOrderAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</>}
            </div>
          </div>
          <div className="cd-card">
            <div className="cd-card-l">PRODUCTS</div>
            <div className="cd-card-v">{row.productCount}</div>
            <div className="cd-card-sub">
              {row.liveCount} live · {row.draftCount} draft
            </div>
          </div>
          <div className="cd-card cd-card-wide">
            <div className="cd-card-l">ORDER STATUS</div>
            {Object.keys(statusCounts).length === 0 ? (
              <div className="cd-card-sub">No orders yet.</div>
            ) : (
              <div className="cd-status-row">
                {Object.entries(statusCounts).map(([k, v]) => (
                  <span key={k} className="cd-status-chip">
      ...
      {tab === "orders" && (
        recentOrders.length === 0 ? (
          <div className="empty">No orders synced for this brand yet.</div>
        ) : (
          <table className="cl-table cd-orders-table">
            <thead><tr>
              <th>Created</th><th>Status</th><th className="num">Total</th>
            </tr></thead>
            <tbody>
              {recentOrders.map((o, i) => (
                <tr key={i}>
                  <td className="cl-date">{o.shopify_created_at ? new Date(o.shopify_created_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}</td>
                  <td><span className="cd-status-chip"><span>{o.pod_status || "new"}</span></span></td>
                  <td className="num mono">₹{Number(o.total_price || 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === "products" && (
        row.products.length === 0 ? (
          <div className="empty">No products added for this brand yet.</div>
        ) : (
          <table className="cl-table">
            <thead><tr>
            ...
            <tbody>
              {row.products.map((p, i) => (
                <tr key={i}>
                  <td><span className="cd-status-chip"><span>{p.status || "—"}</span></span></td>
                  <td className="cl-date">{p.created_at ? new Date(p.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "2-digit" }) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === "team" && (
        row.team.length === 0 ? (
          <div className="empty">No team members linked to this tenant.</div>
        ) : (
          <table className="cl-table">
            <thead><tr>
            ...
            <tbody>
              {row.team.map((m, i) => (
                <tr key={i}>
                  <td>{m.name || "—"}</td>
                  <td>{m.email}</td>
                  <td><span className="cd-status-chip"><span>{m.role}</span></span></td>
                  <td className="cl-date">{m.created_at ? new Date(m.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "2-digit" }) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
.cl-search input { flex: 1; background: none; border: 0; outline: 0; color: var(--ink); font-size: 13px; }
.cl-search svg { color: var(--muted); }
.cl-filters { display: flex; gap: 6px; }
.cl-fchip {
  padding: 7px 12px; border-radius: 999px;
  background: var(--bg-soft); border: 1px solid var(--border);
  color: var(--muted); font-size: 11px; font-weight: 700; letter-spacing: 0.06em;
  text-transform: uppercase; cursor: pointer; transition: all 0.12s ease;
}
.cl-fchip:hover { color: var(--ink); }
.cl-fchip.on { background: var(--ink-yellow-soft, rgba(245, 158, 11, 0.15)); color: var(--ink-yellow, #f59e0b); border-color: var(--ink-yellow, #f59e0b); }
.cl-panel { padding: 0; overflow: hidden; }
...
.cl-table tbody td.num { text-align: right; }
.cl-table tbody tr:last-child td { border-bottom: 0; }
.cl-row { cursor: pointer; transition: background 0.1s ease; }
.cl-row:hover { background: var(--bg-soft); }
.cl-row-chev { color: var(--muted); width: 24px; text-align: right; }
.cl-row:hover .cl-row-chev { color: var(--ink); }
.cl-brand { display: flex; align-items: center; gap: 10px; }
.cl-brand-mark {
  width: 32px; height: 32px; border-radius: 8px;
  background: var(--ink-yellow-soft, rgba(245, 158, 11, 0.2));
  color: var(--ink-yellow, #f59e0b);
  display: grid; place-items: center;
  font-size: 13px; font-weight: 800;
  flex-shrink: 0;
}
.cl-brand-mark-lg { width: 44px; height: 44px; font-size: 18px; border-radius: 10px; }
.cl-brand-name { font-weight: 600; color: var(--ink); }
.cl-brand-slug { font-size: 11px; color: var(--muted); font-family: ui-monospace, SFMono-Regular, monospace; }
.cl-owner-name { font-weight: 500; color: var(--ink); }
.cl-owner-email { font-size: 11px; color: var(--muted); }
.cl-owner-more { font-size: 10px; color: var(--muted); margin-top: 2px; }
.cl-muted { color: var(--muted); font-size: 12px; font-style: italic; }
.cl-date { font-size: 12px; color: var(--muted); white-space: nowrap; }
.cl-shop {
  display: inline-flex; align-items: center; gap: 6px;
  font-size: 12px; color: var(--ink); font-family: ui-monospace, SFMono-Regular, monospace;
}
.cl-shop-none {
...
.cl-dot-ok { background: #34d399; }
.cl-draft-tag { font-size: 11px; color: var(--muted); }

/* Drill-down modal */
.cd-head { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 18px; }
.cd-head-l { display: flex; align-items: center; gap: 14px; }
.cd-name { font-size: 18px; font-weight: 700; color: var(--ink); }
.cd-meta { display: flex; gap: 6px; font-size: 12px; color: var(--muted); margin-top: 4px; flex-wrap: wrap; }
.cd-sep { opacity: 0.5; }
...
.cd-tab.on { color: var(--ink-yellow, #f59e0b); border-bottom-color: var(--ink-yellow, #f59e0b); }
.cd-grid {
...
.cd-card-wide { grid-column: 1 / -1; }
.cd-card-l { font-size: 10px; letter-spacing: 0.1em; font-weight: 700; color: var(--muted); text-transform: uppercase; margin-bottom: 6px; }
.cd-card-v { font-size: 16px; font-weight: 700; color: var(--ink); }
.cd-card-sub { font-size: 12px; color: var(--muted); margin-top: 4px; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.cd-card-link {
  display: inline-flex; align-items: center; gap: 4px;
  font-size: 11px; color: var(--ink-yellow, #f59e0b);
  text-decoration: none; margin-top: 6px;
}
.cd-card-link:hover { text-decoration: underline; }
.cd-status-row { display: flex; flex-wrap: wrap; gap: 6px; }
.cd-status-chip {
  display: inline-flex; align-items: center; gap: 6px;
...
.cd-status-chip strong { font-weight: 700; }
.cd-status-chip span { color: var(--muted); text-transform: uppercase; font-size: 10px; letter-spacing: 0.05em; }
.cd-orders-table { margin-top: 4px; }
